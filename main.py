...
def run_pipeline():
    ...
    print(f"    ASN: +{len(asn_diff['added'])}, -{len(asn_diff['removed'])}")
    ...
    print(f"    Prefix: +{len(prefix_diff['added'])}, -{len(prefix_diff['removed'])}")

    # Step 3: Enumerate IPs
    print("\n[3/6] Enumerating IPs...")
    new_ips = collect_ips(new_prefixes, max_per_prefix=256)
    save_ips(new_ips)
    print(f"    Total IPs: {len(new_ips)}")

    # Step 4: HTTP Scan
    print("\n[4/6] Scanning HTTP services...")
    import asyncio
    ipv4_ips = [ip for ip in new_ips if ip.get("ip_version") == 4]
    new_http = asyncio.run(scan_ips(ipv4_ips[:500]))  # Batasi 500 IP
    save_http_results(new_http)
    print(f"    Alive services: {len(new_http)}")
    ...
    import asyncio
    screenshot_results = asyncio.run(capture_all(new_http, max_screenshots=50))
    save_screenshot_results(screenshot_results)
    print(f"    Screenshots: {len(screenshot_results)}")
